...
// ============================================================
// DEFINITIONS MISSING FROM THE NEW LK
// ============================================================

// Basic constants
#define BLK_SIZE            512
#define BIMG_HEADER_SZ      2048
#define MKIMG_HEADER_SZ     512
#define KB                  1024
...
#define PART_BLKS_RECOVERY  4096

// Define CFG_BOOTIMG_LOAD_ADDR if not defined
#ifndef CFG_BOOTIMG_LOAD_ADDR
#define CFG_BOOTIMG_LOAD_ADDR  0x80008000
#endif

// Define BOOT_MAGIC if not defined
#ifndef BOOT_MAGIC
...
extern part_t* mt_part_get_partition(const char *name);
...
// Kernel start addr (not include any debug header)
unsigned int g_kmem_off = 0;

// Rootfs start addr (not include any debug header)
...
int mboot_recovery_load_raw_part(char *part_name, unsigned long *addr, unsigned int size)
{
    ...
    printf("[%s] Load '%s' partition to 0x%08lX (%d bytes in %ld ms)\n", 
